...
export class SDSConnection extends EventEmitter {
	...
	public connect(clientName: string, host: string, port: number = 11000): Promise<number> {
		return new Promise((resolve, reject) => {
			...
			this.socket.on("connect", async () => {
				...
				response = await this.send(Buffer.from(`${clientName} on ${os.platform()}`));
	...
	private scanParseAndEmit(data: Buffer) {
		if (data.equals(SDSConnection.INVALID)) {
			...
			this.emit("response", new SDSResponse(data));
		} else {
			...
			if (this.bufferedMessageBytes === this.messageSize) {
				...
				this.emit("response", new SDSResponse(this.message.slice(0, this.messageSize)));
	 ...
	 */
	private send(request: SDSRequest | Buffer, waitForResponse: boolean = true): Promise<SDSResponse> {
		if (!this.isConnected) {
